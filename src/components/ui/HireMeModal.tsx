    ...
    if (step === 1) {
      ...
      try {
        ...
        if (!verification.valid) {
          setErrors({
            email: verification.reason || "This email cannot receive mail.",
          });
          return;
        }
        setFormData((current) => ({
          ...current,
          email: verification.normalizedEmail || current.email.trim(),
        }));
      } catch (error) {
        setErrors({
          email:
            ...
            <form onSubmit={handleSubmit} className="p-8">
              {step === 1 && (
                ...
                >
                  ...
                  >
                    {emailChecking ? "Checking Email..." : "Proceed to Project"}
                  </button>
                </motion.div>
              )}
                ...
                >
                  {formData.selectedService && (
                    <div className="bg-red-600/10 border border-red-500/20 rounded-2xl p-4 sm:p-5 flex justify-between items-center">
                      <div>
                        <p className="text-[9px] sm:text-[10px] font-mono uppercase tracking-widest text-red-500/60 mb-1">
                          Selected Service
                        </p>
                        <p className="text-xs sm:text-sm font-bold text-white uppercase italic tracking-wider">
                          {formData.selectedService}
                        </p>
                      </div>
                  ...
                  <div className="space-y-4">
                    <label className="flex items-start gap-4 cursor-pointer group">
                      <div className="relative mt-1">
                        ...
                        />
                        <div
                          className={`w-5 h-5 rounded border ${formData.agreed ? "bg-red-600 border-red-600" : "border-white/20"} transition-all`}
                        />
                        {formData.agreed && (
                          <ShieldCheck
                            size={12}
                      ...
                      <span className="text-[10px] font-mono uppercase tracking-widest text-white/30 group-hover:text-white/50 transition-colors">
                        I agree to the terms and project consultation
                        conditions.
                      </span>
                    </label>
                    {errors.agreed && (
                      <p className="text-red-500 text-[9px] font-mono tracking-widest uppercase">
                        {errors.agreed}
                      </p>
                    )}
                    {errors.details && (
              ...
              {step === 4 && (
                ...
                >
                  ...
                  <h4 className="text-3xl font-bold italic uppercase tracking-tighter text-white">
                    Mission Dispatched
                  </h4>
                  <p className="text-white/40 font-mono text-xs tracking-widest leading-loose">
                    Zubair has been notified. <br />
                    Expect a resonance within 24 hours.
                  </p>
                  <button
                    onClick={onClose}
