...
const GlobeScene = dynamic(
  () =>
    import("@/components/3d/Globe").then((mod) => {
      const { Globe } = mod;
...
export default function Testimonials() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasProfanity(formData.text))
      newErrors.text = "Please Keep it professional";
    ...
    try {
      ...
      if (!db) {
        setErrors({ submit: "Feedback service is unavailable." });
        return;
      }
      ...
      const docData = {
        ...
        text: formData.text,
        email: verification.normalizedEmail || formData.email.trim(),
        rating: 5,
                    ...
                    <div className="bg-white/[0.03] border border-white/10 backdrop-blur-3xl rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 md:p-14 h-full flex flex-col justify-between shadow-2xl">
                      <div>
                        <div className="flex gap-1 mb-8">
                          {Array.from({
                            length: reviews[current]?.rating || 5,
                          }).map((_, i) => (
                            <Star
                              key={i}
                              size={16}
              ...
              {submitted ? (
                <div className="py-10 text-center space-y-6">
                  ...
                  <p className="text-sm font-mono text-white/40 uppercase tracking-[0.2em] leading-relaxed">
                    Your words have been encrypted and sent for verification.{" "}
                    <br />
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <label className="text-[10px] font-mono uppercase tracking-[0.3em] text-white/30 ml-1">
                        Identity
                      </label>
                      <input
                        type="text"
