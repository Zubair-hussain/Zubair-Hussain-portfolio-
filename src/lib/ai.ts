...
export const suggestTimeline = async (
  category: string,
  userLocation: string,
) => {
  try {
    ...
    return (
      (data?.suggestion as string) || "2-4 weeks (Based on project complexity)"
    );
  } catch {
...
export { validateEmailSyntax as validateEmail } from "@/lib/email-verification";
